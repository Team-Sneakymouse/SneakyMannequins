...
/**
 * Serializable snapshot of a mannequin's customization state. Stored as JSON in the sessions/ or
 * templates/ directory.
 */
    ...
    companion object {
        fun fromSelection(sel: LayerSelection): LayerSessionData {
            return LayerSessionData(
                    ...
                    channelColors =
                            sel.channelColors.mapKeys { it.key.toString() }.mapValues {
                                colorToHex(it.value)
                            },
                    texturedColors =
                            sel.texturedColors.mapKeys { it.key.toString() }.mapValues { (_, subMap)
                                ->
                                subMap.mapKeys { it.key.toString() }.mapValues {
                                    colorToHex(it.value)
                                }
                            },
...
fun colorToHex(c: Color): String = String.format("#%02X%02X%02X", c.red, c.green, c.blue)
