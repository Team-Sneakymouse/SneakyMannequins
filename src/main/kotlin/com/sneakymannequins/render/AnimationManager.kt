...
data class RenderSettings(
        ...
        /** If true, the BUILD animation goes top-to-bottom instead of bottom-to-top. */
 ...
 * - **BUILD** mode queues pixels into [BuildAnimation]s that gradually reveal the mannequin from
 * bottom to top, one column-step per tick interval.
 *
 * When a new edit arrives while a BUILD animation is still in progress, any pending (unsent) pixels
 * from the old animation that overlap with the new edit are cancelled — the new animation takes
 * ownership of those pixels. The old animation continues for its remaining non-overlapping pixels.
 */
class AnimationManager(private val plugin: SneakyMannequins, private val handler: VolatileHandler) {
     ...
     * - [RenderMode.BUILD]: queues a new [BuildAnimation], cancelling any overlapping pixels from
     * earlier animations for the same mannequin.
     */
    fun deliver(
            viewer: Player,
            mannequinId: UUID,
        ...
        if (plugin.config.getBoolean("plugin.debug", false)) {
            plugin.logger.info(
                    "[DEBUG] AnimationManager deliver: mannequin=$mannequinId, viewer=${viewer.name}, pixels=${projected.size}, mode=${settings.mode}"
            )
        }
    ...
    private fun queueBuild(
        ...
        playerAnims.filter { it.mannequinId == mannequinId }.forEach { it.cancelPixels(newIndices) }
        playerAnims.removeIf { it.isComplete() }

        // Start the new animation
        val anim =
                BuildAnimation.create(
    ...
    /**
     * Called every server tick. Advances all active BUILD animations and sends the resulting pixels
     * to each viewer.
     */
    private fun tick() {
        ...
        for ((playerUUID, anims) in animations) {
            ...
            while (iter.hasNext()) {
                ...
                if (result.pixels.isNotEmpty()) {
                    ...
                    if (plugin.config.getBoolean("plugin.debug", false)) {
                        plugin.logger.info(
                                "[DEBUG] AnimationManager tick: sending ${scaled.size} pixels for mannequin ${anim.mannequinId} to ${player.name}"
                        )
                    }
                ...
                if (anim.isComplete()) {
                    if (plugin.config.getBoolean("plugin.debug", false)) {
                        plugin.logger.info(
                                "[DEBUG] AnimationManager: animation complete for mannequin ${anim.mannequinId} to ${player.name}"
                        )
                    }
                    iter.remove()
                }
     ...
     * the viewer's scale is 1.0 (the common case).
     */
    private fun scaleForViewer(pixels: List<ProjectedPixel>, viewer: Player): List<ProjectedPixel> {
