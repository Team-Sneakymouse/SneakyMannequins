...
class VolatileHandler1214(private val plugin: SneakyMannequins) : VolatileHandler {
    ...
    override fun applyProjectedPixelsAnimated(
        ...
        if (plugin.config.getBoolean("plugin.debug", false)) {
            ...
            plugin.logger.info(
                    "[NMS] applyPixelChanges viewer=${viewer.name} mannequin=$mannequinId projected=$visibleCount flyIns=${flyInOffsets.size}"
            )
            projected.take(3).forEach {
                plugin.logger.info(
                        "[NMS]   proj idx=${it.index} pos=(${it.x},${it.y},${it.z}) yaw=${it.yaw} argb=${it.argb.toString(16)}"
                )
            }
        }
        ...
        renderManager.handleProjectedPixels(
                ...
                spawn = { proj, entityId ->
                    ...
                    // The TextDisplay background is rendered slightly to the right of
                    // where
                    ...
                    if (flyIn != null) {
                        // ── Fly-in spawn: start at a distant offset with tumble
                        // rotation ──

                        // 1) Spawn with offset translation + random rotation
                        // (instant — duration 0)
                        val offsetTranslation =
                                Vector3f(baseTranslation)
                                        .add(flyIn.offsetX, flyIn.offsetY, flyIn.offsetZ)
                        // Compose the pixel's base rotation with random tumble
                        // angles
                        val tumbledRotation =
                    ...
                    connection.send(
                            ClientboundSetEntityDataPacket(entityId, display.entityData.packAll())
                    )

                    if (flyIn != null || isRiseUp) {
                        // 2) Schedule the target transformation for the next tick
                        // so the
                        //    client has a frame to register the offset as the "old"
                        // position
                        //    before interpolation begins.  We reuse the original
                        // `display`
                        //    object (captured by the lambda) so packAll() re-sends
                        // all
                    ...
                    if (plugin.config.getBoolean("plugin.debug", false) && proj.index == 0) {
                        plugin.logger.info(
                                "[NMS] spawned first pixel entityId=$entityId pos=(${proj.x},${proj.y},${proj.z}) argb=${argb.toString(16)}"
                        )
                    }
                },
                remove = { proj, entityId ->
                    ...
                    if (flyOut != null) {
                        ...
                        val scale = Vector3f(sw * 2f, sh, sh)

                        val offsetTranslation =
                                Vector3f(baseTranslation)
                                        .add(flyOut.offsetX, flyOut.offsetY, flyOut.offsetZ)
                        val tumbledRotation =
                                Quaternionf(rotation)
                                        .rotateX(flyOut.rotX)
                                        .rotateY(flyOut.rotY)
                        ...
                        display.setTransformationInterpolationDelay(0)
                        display.setTransformationInterpolationDuration(flyOut.interpolationTicks)

                        display.setText(Component.literal(" "))
                        display.setTextOpacity(0.toByte())
                        display.setWidth(1f)
                        display.setHeight(1f)
                        display.entityData.set(TextDisplay.DATA_BACKGROUND_COLOR_ID, proj.argb)
                        ...
                        plugin.server.scheduler.scheduleSyncDelayedTask(
                                ...
                                flyOut.interpolationTicks.toLong() + 1
                        )
                    } else {
                        connection.send(ClientboundRemoveEntitiesPacket(*intArrayOf(entityId)))
                    }
                },
    ...
    override fun destroyMannequin(viewer: Player, mannequinId: UUID) {
        ...
        renderManager.destroyMannequin(viewer, mannequinId) { ids ->
            ...
            if (plugin.config.getBoolean("plugin.debug", false)) {
                plugin.logger.info(
                        "[NMS] destroyMannequin viewer=${viewer.name} mannequin=$mannequinId removed=${ids.size}"
                )
            }
        }
    }
